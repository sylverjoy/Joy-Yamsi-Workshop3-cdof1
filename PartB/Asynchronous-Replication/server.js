...
// Middleware to periodically replicate data to secondary storage (every 5 minutes)
setInterval(() => {
    replicateData();
}, 300000); 
...
// Replicate data to secondary storage
function replicateData() {
    try {
        try {
            if (fs.existsSync('products.json')) {
                const products_to_replicate = JSON.parse(fs.readFileSync('products.json', 'utf8'));
            } else {
                const products_to_replicate = [];
            }
        } catch (err) {
            console.error('Error loading products data:', err);
            const products_to_replicate = [];
        ...
        try {
            if (fs.existsSync('orders.json')) {
                const orders_to_replicate = JSON.parse(fs.readFileSync('orders.json', 'utf8'));
            } else {
                const orders_to_replicate = [];
            }
        } catch (err) {
            console.error('Error loading orders data:', err);
            const orders_to_replicate = [];
        ...
        try {
            if (fs.existsSync('carts.json')) {
                const carts_to_replicate = JSON.parse(fs.readFileSync('carts.json', 'utf8'));
            } else {
                const carts_to_replicate = {};
            }
        } catch (err) {
            console.error('Error loading carts data:', err);
            const carts_to_replicate = {};
        ...
        fs.writeFileSync('products_mirror.json', JSON.stringify(products_to_replicate, null, 2), 'utf8');
        fs.writeFileSync('orders_mirror.json', JSON.stringify(orders_to_replicate, null, 2), 'utf8');
        fs.writeFileSync('carts_mirror.json', JSON.stringify(carts_to_replicate, null, 2), 'utf8');
        
        console.log('Data replicated to secondary storage successfully');
    } catch (err) {
